...
**Prerequisite:** [Semantic Memory](https://strangerloops.com/semantic-memory.md) — build the system first, then come back here.
...
1. **Add ⚠️ MANDATORY section** to top of AGENTS.md (right after the title)
2. **Include specific examples** with actual commands, not just "search for context"
3. **Add `logSearch()` function** to your search CLI — logs to `logs/tpuf-searches-YYYY-MM-DD.log`
4. **Create evaluation tests** with obscure facts only findable via search
5. **Verify after subagent runs** — `cat logs/tpuf-searches-$(date +%Y-%m-%d).log | tail -20`
6. **Iterate** — if searches aren't happening, make the instruction more explicit
